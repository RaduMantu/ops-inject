...
# anayze_icmp-ts.sh - extract ip-timestamp reachability and path
...
while read -r PROVIDER REGION EXT_IP; do
    ...
    while read -r PKT_ID SRC_IP; do
        ...
        # fetch overflow before taking a closer look at each ip
        OVF=$(tshark -r ${PCAP_FILE}                \
                     -Y "frame.number==${PKT_ID}"   \
                     -e ip.opt.overflow             \
                     -T ek                          \
              | tail -n 1                           \
              ...
              | tr -d '"')
        printf "\033[33;1m  >> ovf:\033[2m %s\033[0m\n" ${OVF:-0}

        # for each ip in timestamp option
        for TS_IP in $(tshark -r ${PCAP_FILE}                    \
                              -Y "frame.number==${PKT_ID}"       \
                              -e ip.opt.time_stamp_addr          \
                              -T ek                              \
                       | tail -n 1                               \
                       | jq -Ma '.layers.ip_opt_time_stamp_addr' \
                       | tr -d '",[]'                            \
                       | xargs); do
